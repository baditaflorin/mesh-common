...
| **`useFileShare`** 🆕                                                 | **Chunked file share through the existing Yjs transport (5 MB cap; tune `chunkBytes`). Writes yield every eight chunks by default so mobile browsers stay responsive. Receiver gets `download(fileId)` and `blobOf(fileId)`.**                                                                                                                                                                                               |
