...
describe("fifth primitive wave", () => {
  it("tracks provider lifecycle and reconnects when the provider supports it", () => {
    ...
    const roomWithProvider = { ...room, provider: { connected: false, connect, disconnect, on: (_: string, callback: (event: { status?: string }) => void) => listeners.add(callback), off: (_: string, callback: (event: { status?: string }) => void) => listeners.delete(callback) } };
    const { result } = renderHook(() => useRoomLifecycle(roomWithProvider));
    expect(result.current.status).toBe("joining");
    act(() => listeners.forEach((listener) => listener({ status: "connected" })));
