...
const rules = [
  ...
  [/(doorbell|clap-track|tap-symphony|room-soundtrack|dj-deck|mic-drop|metronome)/, "Devices & spaces", "Audio & haptics", ["sound", "performances", "co-located groups"]],
  [/(find-my-family|route-share|treasure-hunt|crowd-map)/, "Devices & spaces", "Location & field tools", ["outdoors", "neighbourhoods", "events"]],
  [/(kiosk|watch-party|shared-window|lightning|firefly)/, "Devices & spaces", "Shared displays & installations", ["installations", "shared screens", "co-located groups"]],
  // Games split by the actual interaction rather than one catch-all bucket.
  [/(mafia|werewolf|spyfall|codenames|taboo|heads-up|truths|would-rather|never-have|fortune-cookie|questions-only|name-game)/, "Games", "Conversation & bluffing", ["icebreakers", "game night", "small groups"]],
  [/(rps|tic-tac|snake|laser-tag|marble|tug-of-war|spot-it|memory-match|quick-draw|speed-type)/, "Games", "Arcade & reaction", ["game night", "two players", "quick rounds"]],
  [/(bingo|trivia|quiz|pictionary|hot-potato|five-second|word-chain|dice|jellybean|dare-wheel|pass-the-phone|sound-guess)/, "Games", "Party & tabletop", ["icebreakers", "game night", "small groups"]],
  ...
  [/(decision-room|picker|lunch-roulette|book-club-lottery)/, "Decisions", "Selection & ranking", ["team decisions", "choices", "small groups"]],
  ...
  [/(privacy|2fa|passport|secret|anonymous|handshake|gift-exchange|trade-cards|favor-bank|allowance)/, "Privacy & utilities", "Identity & private exchange", ["private exchange", "onboarding", "personal tools"]],
  [/(clipboard|link-share|network-builder|qr)/, "Privacy & utilities", "Links, QR & transfer", ["sharing", "onboarding", "personal tools"]],
];
