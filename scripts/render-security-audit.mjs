...
md.push("");
md.push(`> A programmatic, CPU-only verification of shared security invariants and app-specific safety checks.`);
md.push(`> Re-run with \`npm run audit:security\` from this repo. Source: \`mesh-common/tests/securityAudit.test.ts\``);
if (ranUi) md.push(`> + this app's \`tests/e2e/security-audit.spec.ts\` app-specific UI safety checks.`);
else md.push(`> This app has no app-specific UI audit yet — only the shared crypto invariants are exercised. The layer-1 guarantees still apply by virtue of bundling \`mesh-common\`.`);
md.push("");
md.push(`## Result`);
md.push("");
...
md.push(`- UI-flow checks: **${uiCount}**${ranUi ? "" : "  _(no app-specific UI audit is present; pass 2 skipped)_"}`);
md.push("");
md.push(`## Checks`);
md.push("");
...
md.push("2. **UI flow** (Playwright, app-specific) — opens the browser scenario declared in `tests/e2e/security-audit.spec.ts` and verifies the app's own safety contract.");
md.push("");
